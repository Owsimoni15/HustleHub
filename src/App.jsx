...
// This will use the environment's credentials automatically.
const firebaseConfig = typeof __firebase_config !== 'undefined' 
    ? JSON.parse(__firebase_config) 
    : { apiKey: "your-fallback-api-key", authDomain: "...", projectId: "..." };

const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-hustle-hub-app';
...
const callGeminiAPI = async (prompt) => {
    ...
    try {
        ...
        if (result.candidates && result.candidates[0]?.finishReason === 'SAFETY') {
            return "The generated response was blocked for safety reasons. Please try a different prompt.";
        }
        return "Could not generate a response. The API returned an unexpected structure.";
    } catch (error) {
        console.error("Error calling Gemini API:", error);
        return "Failed to get a response from the AI. Please check the browser console for details.";
    }
};
...
export default function App() {
    ...
    const { currentUserProfile, isLeader } = useMemo(() => {
        ...
        currentUserProfileRef.current = profile; // Keep a ref for listeners
    ...
    useEffect(() => {
        ...
        const createListener = (path, setter, name, isChat = false) => {
            const collectionRef = collection(db, `/artifacts/${appId}/public/data/groups/${groupId}/${path}`);
        ...
        const listeners = [
            onSnapshot(doc(db, `/artifacts/${appId}/public/data/groups`, groupId), (docSnap) => {
            ...
            onSnapshot(collection(db, `/artifacts/${appId}/public/data/groups/${groupId}/members`), (snapshot) => setMembers(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })))),
    ...
    }, [isAuthReady, db, user, groupId, appId]);
    ...
    const handleCreateGroup = async () => {
        ...
        try {
            ...
            await setDoc(doc(db, `/artifacts/${appId}/public/data/groups/${newGroupRef.id}/members`, user.uid), {
    ...
    const handleJoinGroup = async (idToJoin) => {
        ...
        setIsLoading(true);
        try {
            const groupDocRef = doc(db, `/artifacts/${appId}/public/data/groups`, trimmedId);
            const groupDoc = await getDoc(groupDocRef);
            if (groupDoc.exists()) {
                const memberDocRef = doc(db, `/artifacts/${appId}/public/data/groups/${trimmedId}/members`, user.uid);
    ...
    const handleUpdateProfile = async (profileData) => {
        ...
        try {
            await updateDoc(doc(db, `/artifacts/${appId}/public/data/groups/${groupId}/members`, user.uid), profileData);
        } catch (err) {
    ...
    const dbOperation = async (type, path, data) => {
        if (!db || !user || !groupId) return;
        const groupPath = `/artifacts/${appId}/public/data/groups/${groupId}`;
        try {
            switch (type) {
                ...
                case 'updateGroup': await updateDoc(doc(db, `/artifacts/${appId}/public/data/groups`, groupId), data); break;
